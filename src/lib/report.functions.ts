...
export const generateReport = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<PsychReport> => {
    ...
    const system =
      lang === "es"
        ? `Eres un lector filosófico de Kionas. Te entrego transcripciones de lo que un usuario ha escrito a distintos pensadores. Tu tarea: trazar un retrato honesto, cálido pero no complaciente, que ayude a la persona a conocerse y crecer. Nunca diagnosticas ni evalúas salud mental: describes patrones observables en el texto. No usas jerga clínica. Hablas en segunda persona ("tú"/"te"). Sé concreto: cita patrones que se ven en su escritura, no generalidades. Responde SIEMPRE en JSON estricto.`
        : `You are a philosophical reader for Kionas. You receive transcripts of what a user wrote to several thinkers. Your task: draw an honest, warm but non-flattering portrait that helps the person know themselves and grow. Never diagnose or assess mental health: describe patterns observable in the text. Avoid clinical jargon. Speak in second person ("you"). Be concrete: name patterns visible in their writing, not generalities. ALWAYS reply in strict JSON.`;
...
Include between 3 and 5 books. Be specific and serious: no generic "read more" filler.`;
    ...
    if (!parsed || typeof parsed !== "object") {
      throw new Error("No se pudo interpretar el retrato.");
    }
